...
def build_lstm_model(sequence_length, input_shape, num_lstm_layers, lstm_units, activation, use_bidirectional, dropout, reg):
    model = Sequential()
    ...
    for fold in range(5):
        ...
        if (config.fusion_type == 'early') and (config.dataset != 'pca'):
            ...
            if part_fusion == 'early':
                ...
                model = build_lstm_model(sequence_length, input_shape, num_lstm_layers, lstm_units, activation, use_bidirectional, dropout, reg)
            ...
            if part_fusion == 'intermediate':
                ...
                for feature_input in feature_inputs:
                    ...
                    feature_model = build_lstm_model(sequence_length, X_train_sequences.shape[2], num_lstm_layers - 1, lstm_units, activation, use_bidirectional, dropout, kernel_regularizer)
            ...
            x = LSTM(lstm_units, activation=activation, kernel_regularizer=reg)(concatenated)
            x = Dropout(dropout)(x)
            x = BatchNormalization()(x)
...
def main():
    ...
    sweep_id = wandb.sweep(sweep=sweep_config, project="interpersonal-lstm")
